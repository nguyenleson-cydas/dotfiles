-- Credit: https://github.com/hendrikmi/dotfiles/blob/main/nvim/lua/tools/sql-runner.lua
local M = {}
...
local function run_query(name, cmd)
  ...
  local result = vim.fn.system(cmd, sql_text)

  -- Check for errors
  if vim.v.shell_error ~= 0 then
    vim.notify('[sql-runner] Command failed: ' .. result, vim.log.levels.ERROR)
    return
  end

  -- Ensure cache directory exists and write output to file
  vim.fn.mkdir(vim.fn.stdpath 'data' .. '/sql-runner', 'p')
  local file = io.open(outfile, 'w')
  if file then
    file:write(result)
    file:close()
  else
    vim.notify('[sql-runner] Failed to write output file', vim.log.levels.ERROR)
    return
  end
  ...
  -- Set nowrap, readonly, and modifiable=false on the results buffer
